...
const shouldProxyFileProtocol = (() => {
  if (typeof window === 'undefined' || typeof window.location === 'undefined') return false;
  return window.location.protocol === 'file:' && !!CONFIG.fileProtocolProxyPrefix;
})();

const buildFetchUrl = url => {
  if (!shouldProxyFileProtocol) return url;
  const stripped = url.replace(/^https?:\/\//, '');
  return `${CONFIG.fileProtocolProxyPrefix}${stripped}`;
};
